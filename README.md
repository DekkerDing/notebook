...
| **分布式锁** | 可重入锁、读写锁、公平锁、锁回调模板 | ✅ 已实现 |
| **两级缓存** | L1 Caffeine + L2 Redis，缓存加载器、统计信息 | ✅ 已实现 |
| **限流** | 令牌桶算法、限流回调、预定义限流器 | ✅ 已实现 |
| **ID生成** | 序列ID、业务ID、雪花算法、时间戳ID | ✅ 已实现 |
| **延时队列** | 时间轮算法 + SortedSet混合实现 | ✅ 已实现 |
| **地理服务** | GEO命令封装、位置添加、坐标查询 | ✅ 已实现 |
| **基数统计** | HyperLogLog UV统计、DAU/WAU统计 | ✅ 已实现 |
| **布隆过滤器** | 误判判断、去重、缓存穿透防护 | ✅ 已实现 |
| **Lua脚本** | 脚本加载、执行、缓存 | ✅ 已实现 |
